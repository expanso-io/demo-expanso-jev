...
const baseState={mode:'live',apply_enabled:true,cluster_context:'test-fixture',namespace:'jev-label-demo',cloud:{state:'running'},pods:['analytics-reference','checkout-new','checkout-reference'].map(name=>({name,namespace:'jev-label-demo',event_enabled:true,node:'test-node',labels:{app:name.includes('analytics')?'analytics':'checkout'},logs:[]})),events:[]};
...
(async()=>{await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const browser=await chromium.launch({headless:true,channel:'chrome'});try{
 ...
 let state=structuredClone(baseState),events=[],seq=0,posted=[];
 ...
 const target=page.locator('[data-pod="jev-label-demo/checkout-new"]');await target.click();
 ...
 await page.waitForFunction(()=>document.querySelector('[data-pod="jev-label-demo/checkout-new"]').textContent.includes('routing-tier=stable'));
 console.log('PASS immediate injection, five-leg particle flow, and confirmation-gated labels');
 for(const [pod,scenario] of [['analytics-reference','analytics'],['checkout-reference','security']]){await page.locator(`[data-scenario="${scenario}"]`).click();await page.locator(`[data-pod="jev-label-demo/${pod}"]`).click();await page.waitForFunction(p=>document.getElementById('result-title').textContent.startsWith(p+': label added'),pod);}
 assert.deepEqual(posted.map(e=>[e.pod,e.scenario]),[['checkout-new','checkout'],['analytics-reference','analytics'],['checkout-reference','security']]);
