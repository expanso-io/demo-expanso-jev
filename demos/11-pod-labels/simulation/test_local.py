...
class LauncherTests(unittest.TestCase):
    ...
    def test_fixture_upgrade_preserves_current_and_unrelated_pods(self):
        ...
        old["metadata"]["annotations"]["jev.expanso.io/workload-version"] = "signals-v7"
...
class InvestigationFixtureTests(unittest.TestCase):
    def test_scenarios_emit_synthetic_stdout_without_label_proposals(self):
        import json

        spec = importlib.util.spec_from_file_location("workload", HERE / "workload.py")
        ...
        for name in (
            ...
            scenario = workload.SCENARIOS[name]
            self.assertEqual(scenario["prefer"], [])
            self.assertEqual(scenario["lane"], "investigation")
            self.assertTrue(scenario["synthetic_stimulus"])
            for field in ("title", "why", "message"):
            ...
            self.assertEqual(event, json.loads(handler.wfile.getvalue()))
            self.assertTrue(event["synthetic_stimulus"])
            self.assertEqual(event["lane"], "investigation")
            self.assertNotIn("prefer", event)
            handler.send_response.assert_called_once_with(200)
        for name in ("investigate_restart", "investigate_context"):
            self.assertNotIn("reason", workload.SCENARIOS[name])
            self.assertNotIn("stack", workload.SCENARIOS[name])
        evidence = workload.SCENARIOS["investigate_evidence"]
        self.assertEqual(evidence["reason"], "OOMKilled")
        self.assertEqual(evidence["exit_code"], 137)
        self.assertEqual(evidence["memory_limit"], "512Mi")
        self.assertIn("OrderCache.load:212", evidence["stack"])
        self.assertIn("cache limit", evidence["recent_change"])

    def test_fixture_embedding_and_external_label_seed(self):
        import textwrap

        fixture = (HERE / "fixtures.yaml").read_text()
        embedded = fixture.split("  workload.py: |\n", 1)[1].split("\n---", 1)[0]
        self.assertEqual(
            textwrap.dedent(embedded) + "\n", (HERE / "workload.py").read_text()
        )
        pods = [doc for doc in fixture.split("\n---\n") if "kind: Pod\n" in doc]
        self.assertEqual(len(pods), 3)
        for pod in pods:
            self.assertIn("jev.expanso.io/workload-version: signals-v7", pod)
            self.assertNotIn("jev.expanso.io/changes:", pod)
        analytics = next(pod for pod in pods if "name: analytics-worker\n" in pod)
        self.assertIn("    routing-tier: batch\n", analytics)
        self.assertIn(
            "  selector:\n    app: checkout\n    routing-tier: stable\n", fixture
        )
